...
def wine_chooser(variety, price):
    ...
    results = conn.execute(f"SELECT title, region, country, points, price FROM wine_table WHERE variety = '{variety}' AND price < {price} ORDER BY points DESC LIMIT 3").fetchall()
